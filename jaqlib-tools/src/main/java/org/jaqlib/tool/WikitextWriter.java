...
public class WikitextWriter
{
  ...
  private static void transform(String queryBuilder) throws IOException {
    URL outputUrl = WikitextWriter.class.getClassLoader().getResource("output/" + queryBuilder + ".txt");
    WikitextWriter writer = new WikitextWriter(outputUrl.getFile());

    URL inputUrl = WikitextWriter.class.getClassLoader().getResource("input/" + queryBuilder + ".javadoc");
    FileReader reader = new FileReader(inputUrl.getFile());
